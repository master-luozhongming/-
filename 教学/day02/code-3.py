...
def show_animation(imgs):  # 显示并保存动画
    """将图片序列转为动画并保存"""
    rc("animation", html="jshtml")  # 设置动画后端
    fig, ax = plt.subplots(1, 1, figsize=(5, 3))  # 创建图形
    frames = []  # 帧列表

    text = ax.text(10, 20, "", fontsize=12, color="black")  # 文本对象（未使用）

    for i, img in enumerate(imgs):  # 遍历图片
        frame = [ax.imshow(img, animated=True)]  # 添加图片帧
    ...
    # 保存动画
    ani.save("cartpole.mp4", writer="ffmpeg")  # MP4 格式
    ani.save("cartpole.gif", writer="pillow")  # GIF 格式
...
def plot_loss(episode_list, return_list, filename):  # 绘制奖励曲线
    """绘制奖励图像"""
    f = plt.figure()  # 创建图形
    plt.plot(episode_list, return_list)  # 绘制曲线
    plt.xlabel("Episodes")  # x 轴标签
    plt.ylabel("Returns")  # y 轴标签
    plt.title("CartPole-v0")  # 标题
    plt.show()  # 显示
    f.savefig(filename, bbox_inches="tight")  # 保存文件
...
class PolicyNet(nn.Module):  # 策略网络
    ...
    def __init__(self, action_size):  # 构造函数
        super().__init__()
        self.l1 = nn.Linear(4, 128)  # 第一层：4 → 128
        self.l2 = nn.Linear(128, action_size)  # 第二层：128 → 动作数

    def forward(self, x):  # x 是 S_t
        x = F.relu(self.l1(x))  # ReLU 激活
        x = F.softmax(self.l2(x), dim=1)  # Softmax 输出概率
        return x
...
class Agent:  # 智能体
    def __init__(self):  # 构造函数
        ...
        self.lr = 0.0002  # 学习率 α
        self.action_size = 2  # 动作数量
        # 初始化策略网络 π_θ
        self.pi = PolicyNet(self.action_size)  # 创建网络
        self.optimizer = optim.Adam(  # 创建优化器
            self.pi.parameters(),
            lr=self.lr
        )
    ...
    def get_action(self, state):  # 选择动作
        """输入参数：环境的状态"""
        # 动作的概率分布
        probs = self.pi(torch.tensor(state).unsqueeze(0)).squeeze(0)  # 状态 → 概率
        # 创建分类分布采样器
    ...
    def collect_trajectory(self, env):  # 收集轨迹
        """在环境 env 中采样一条轨迹"""
        state, _ = env.reset()  # 重置环境（Gymnasium API）
    ...
    def update(self, trajectory):  # 更新网络
        """用轨迹数据更新策略网络（REINFORCE 算法）"""
        states, actions, rewards = trajectory  # 解包轨迹
        # 逆序计算每个时间步的 G_t
        G = 0  # 初始化回报
        loss = 0  # 初始化损失
        for r, s, a in zip(rewards[::-1], states[::-1], actions[::-1]):  # 逆序遍历
            # 计算 G_t = R_t + γ * G_{t+1}
            G = r + self.gamma * G  # 递推计算折扣回报
            _, probs = self.get_action(s)  # 获取状态 s 的动作概率
            log_prob = torch.log(probs)[a]  # 获取动作 a 的对数概率
            loss += - G * log_prob  # 累加损失：-G_t * log π(a_t|s_t)
...
# 可视化
plot_loss(episode_list, return_list, "pg-loss.pdf")  # 保存奖励曲线
